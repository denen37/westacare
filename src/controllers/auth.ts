...
export const registerProvider = async (req: Request, res: Response) => {
    ...
    await OTP.create({ email, otp: hash(otp), expiresAt: otpExpiration });
...
export const verifyOTP = async (req: Request, res: Response) => {
    ...
    let otpRecord = await OTP.findOne({ where: { email, otp: hash(otp) } })
...
export const sendOTP = async (req: Request, res: Response) => {
    ...
    let otp = randomId(6).toString();

    let otpExpires = new Date(Date.now() + config.OTP_EXPIRY_TIME * 60 * 1000);

    let otpRecord = await OTP.create({ email, otp: hash(otp), expiresAt: otpExpires })
